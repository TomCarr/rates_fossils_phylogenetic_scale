...
absolute_node_ages_percentage_storage <<- vector(mode="numeric", length=0)
...
#########################################
			...
			if (length(absolute_fossil_ages) > 0){
				...
				absolute_node_ages_percentage_storage <<- append(absolute_node_ages_percentage_storage, node_ages/max(node.depth.edgelength(crown_group_tree))) 
			}
		}	
	}
print(paste("fossils simulated", length(absolute_fossil_ages_storage)))

###

if (length(absolute_fossil_ages_storage) > 0){
percentage_difference <- (absolute_fossil_ages_storage-absolute_node_ages_storage)/absolute_node_ages_storage #correct age compared to the fossil age

intervals <- seq(0, 1, 0.1)
bins <- vector("list", 10)
for (l in 1:length(percentage_difference)){
for (m in 2:length(intervals)){
if ((absolute_node_ages_percentage_storage[[l]] > intervals[[m-1]]) & (absolute_node_ages_percentage_storage[[l]] <= intervals[[m]])){
bins[[m-1]] <- append(bins[[m-1]], percentage_difference[[l]])
}
}
}
...
intervals <- seq(0, 1, 0.1)
bins <- vector("list", 10)
for (a in 1:length(percentage_difference)){
for (b in 2:length(intervals)){
if ((absolute_node_ages_percentage_storage[[a]] > intervals[[b-1]]) & (absolute_node_ages_percentage_storage[[a]] <= intervals[[b]])){
bins[[b-1]] <- append(bins[[b-1]], percentage_difference[[a]])
}
}
}
...
table <- data.frame(bins[[10]])

plot <- ggplot(table, aes(table[,1])) + 
geom_density(fill="cyan", alpha=0.5, weight=0, adjust=2)+
